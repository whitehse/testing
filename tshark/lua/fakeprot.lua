----------------------------------------
FakeProt-- script-name: fakeprot.lua
--
-- author: Hadriel Kaplan <hadrielk at yahoo dot com>
-- fakeprot modifications by Dan White <whitehse@gmail.com>
...
-- Once the script is loaded, it creates a new protocol named "FakeProt".  If you have
...
-- called "FakeProt", select that and click the "ok" or "apply" button.  Voila`, you're now decoding DNS packets
...
-- and since the FakeProt protocol in this script has been set to automatically decode UDP port 65333, it will
...
local dns = Proto("fakeprot","Fakeprot Protocol")
...
local pf_trasaction_id      = ProtoField.new   ("Transaction ID", "fakeprot.trans_id", ftypes.UINT16)
local pf_flags              = ProtoField.new   ("Flags", "fakeprot.flags", ftypes.UINT16, nil, base.HEX)
local pf_num_questions      = ProtoField.uint16("fakeprot.num_questions", "Number of Questions")
local pf_num_answers        = ProtoField.uint16("fakeprot.num_answers", "Number of Answer RRs")
local pf_num_authority_rr   = ProtoField.uint16("fakeprot.num_authority_rr", "Number of Authority RRs")
local pf_num_additional_rr  = ProtoField.uint16("fakeprot.num_additional_rr", "Number of Additional RRs")
...
local pf_flag_response              = ProtoField.new   ("Response", "fakeprot.flags.response", ftypes.BOOLEAN, {"this is a response","this is a query"}, 16, 0x8000, "is the message a response?")
local pf_flag_opcode                = ProtoField.new   ("Opcode", "fakeprot.flags.opcode", ftypes.UINT16, nil, base.DEC, 0x7800, "operation code")
local pf_flag_authoritative         = ProtoField.new   ("Authoritative", "fakeprot.flags.authoritative", ftypes.BOOLEAN, nil, 16, 0x0400, "is the response authoritative?")
local pf_flag_truncated             = ProtoField.bool  ("fakeprot.flags.truncated", "Truncated", 16, nil, 0x0200, "is the message truncated?")
local pf_flag_recursion_desired     = ProtoField.bool  ("fakeprot.flags.recursion_desired", "Recursion desired", 16, {"yes","no"}, 0x0100, "do the query recursivley?")
local pf_flag_recursion_available   = ProtoField.bool  ("fakeprot.flags.recursion_available", "Recursion available", 16, nil, 0x0080, "does the server support recursion?")
local pf_flag_z                     = ProtoField.uint16("fakeprot.flags.z", "World War Z - Reserved for future use", base.HEX, nil, 0x0040, "when is it the future?")
local pf_flag_authenticated         = ProtoField.bool  ("fakeprot.flags.authenticated", "Authenticated", 16, {"yes","no"}, 0x0020, "did the server DNSSEC authenticate?")
local pf_flag_checking_disabled     = ProtoField.bool  ("fakeprot.flags.checking_disabled", "Checking disabled", 16, nil, 0x0010)
...
local pf_flag_rcode         = ProtoField.uint16("fakeprot.flags.rcode", "Response code", base.DEC, rcodes, 0x000F)
local pf_query              = ProtoField.new("Query", "fakeprot.query", ftypes.BYTES)
local pf_query_name         = ProtoField.new("Name", "fakeprot.query.name", ftypes.STRING)
local pf_query_name_len     = ProtoField.new("Name Length", "fakeprot.query.name.len", ftypes.UINT8)
local pf_query_label_count  = ProtoField.new("Label Count", "fakeprot.query.label.count", ftypes.UINT8)
local rrtypes = { [1] = "A (IPv4 host address)", [2] = "NS (authoritative name server)", [28] = "AAAA (for geeks only)" }
local pf_query_type         = ProtoField.uint16("fakeprot.query.type", "Type", base.DEC, rrtypes)
...
local pf_query_class        = ProtoField.uint16("fakeprot.query.class", "Class", base.DEC, classes, nil, "keep it classy folks")
...
local ef_query     = ProtoExpert.new("fakeprot.query.expert", "DNS query message",
                                     expert.group.REQUEST_CODE, expert.severity.CHAT)
local ef_response  = ProtoExpert.new("fakeprot.response.expert", "DNS response message",
                                     expert.group.RESPONSE_CODE, expert.severity.CHAT)
local ef_ultimate  = ProtoExpert.new("fakeprot.response.ultimate.expert", "DNS answer to life, the universe, and everything",
                                     expert.group.COMMENTS_GROUP, expert.severity.NOTE)
-- some error expert info's
local ef_too_short = ProtoExpert.new("fakeprot.too_short.expert", "DNS message too short",
                                     expert.group.MALFORMED, expert.severity.ERROR)
local ef_bad_query = ProtoExpert.new("fakeprot.query.missing.expert", "DNS query missing or malformed",
...
local questions_field       = Field.new("fakeprot.num_questions")
local query_type_field      = Field.new("fakeprot.query.type")
local query_class_field     = Field.new("fakeprot.query.class")
local response_field        = Field.new("fakeprot.flags.response")
...
-- "fakeprot.flags.response" ProtoField to ftype.BOOLEAN way earlier when we created the
...
dns.prefs.port  = Pref.uint("Port number", default_settings.port,
                            "The UDP port number for FakeProt")
...
function dns.prefs_changed()
    ...
    if default_settings.port ~= dns.prefs.port then
        ...
        if default_settings.port ~= 0 then
            dprint2("removing FakeProt from port",default_settings.port)
        ...
        if default_settings.port ~= 0 then
            dprint2("adding FakeProt to port",default_settings.port)
...
dprint2("FakeProt Prefs registered")
...
function dns.dissector(tvbuf,pktinfo,root)
    ...
    pktinfo.cols.protocol:set("FakeProt")
    ...
    -- now add more to the main fakeprot tree
